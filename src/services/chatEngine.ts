import type { MoodState, PetEmotion, PetSpecies } from '@/types'
 ...
 * 本地规则回复表
 *
 * 定位：LLM 不可用时的降级层，同时承担零配置开箱可用。
 * 匹配顺序即数组顺序——越具体的规则越靠前，避免被宽泛规则抢先命中。
 */
...
/** 兜底回复：未命中任何规则时，按当前情绪给出贴合语境的应答 */
const FALLBACK_BY_EMOTION: Readonly<Record<PetEmotion, readonly string[]>> = {
...
export function generateLocalReply(
  ...
  const pool = FALLBACK_BY_EMOTION[ctx.emotion]
 ...
 * 主动搭话题料
 * 由「状态 / 场景」选择器挑选，而非纯随机——
 * 饿的时候说「想吃饭」比随机说「今天天气真好」更可信。
 */
export const PROACTIVE_BY_STATE: Readonly<
  Record<'hungry' | 'sleepy' | 'bored' | 'affectionate' | 'idle', readonly string[]>
> = {
  hungry: [
    '主人主人～我肚子咕咕叫了…',
    '有没有什么好吃的呀？就一点点！',
...
export const PROACTIVE_BY_SCENE: Readonly<
  Record<'coldStart' | 'longAbsence' | 'morning' | 'night', readonly string[]>
> = {
